...
one_min_before = (datetime.utcnow() - timedelta(minutes=120)).isoformat() + 'Z'

# CpuUtilization 메트릭 데이터 요약
try:
    summarize_metrics_data_response = monitoring_client.summarize_metrics_data(
        compartment_id=compartment_id,
        summarize_metrics_data_details=oci.monitoring.models.SummarizeMetricsDataDetails(
            namespace="oci_objectstorage",  # Compute Agent 메트릭 네임스페이스
            query="StoredBytes[5m].mean()",  # 1분 단위 평균값
    ...
    if summarize_metrics_data_response.data:
        ...
        print("No data returned for CpuUtilization.")
